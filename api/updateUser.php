...
if (!empty($password)) {
    ...
    $stmt = $conn->prepare("UPDATE users SET username = ?, first_name = ?, last_name = ?, email = ?, password_hash = ?, role = ? WHERE id = ?");
    $stmt->bind_param("ssssssi", $username, $first_name, $last_name, $email, $password_hash, $role, $id);
} else {
